...
//	typedef struct NewtonRagDollBone{} NewtonRagDollBone;
	...
	#define SERIALIZE_ID_HEIGHFIELD				11			
	#define SERIALIZE_ID_SCENE					12			

	typedef struct NewtonCollisionInfoRecordTag
	{

		typedef struct NewtonBoxTag
		{
			dFloat m_x;
			dFloat m_y;
			dFloat m_z;
		...
		typedef struct NewtonSphereTag
		{
			dFloat m_r0;
			dFloat m_r1;
			dFloat m_r2;
		...
		typedef struct NewtonCylinderTag
		{
			dFloat m_r0;
			dFloat m_r1;
			dFloat m_height;
		...
		typedef struct NewtonCapsuleTag
		{
			dFloat m_r0;
			dFloat m_r1;
			dFloat m_height;
		...
		typedef struct NewtonConeTag
		{
			dFloat m_r;
			dFloat m_height;
		} NewtonConeParam;

		typedef struct NewtonChamferCylinderTag
		{
			dFloat m_r;
			dFloat m_height;
		} NewtonChamferCylinderParam;

		typedef struct NewtonConvexHullModifierTag
		{
			NewtonCollision* m_chidren;
		} NewtonConvexHullModifierParam;


		typedef struct NewtonCompoundCollisionTag
		{
			int m_chidrenCount;
			NewtonCollision** m_chidren;
		...
		union {
			...
			dFloat m_paramArray[14];		    // user define collision can use this to store information
		};
	} NewtonCollisionInfoRecord;

	typedef struct NewtonJointRecordTag
	{
...
//	NEWTON_API int NewtonGetThreadNumber (const NewtonWorld* newtonWorld);
...
//	NEWTON_API void NewtonWorldForEachBodyDo (const NewtonWorld* newtonWorld, NewtonBodyIterator callback);
	...
	NEWTON_API void NewtonCollisionSerialize (const NewtonCollision* collision, NewtonSerialize serializeFunction, void* serializeHandle);
