...
class JOINTLIBRARY_API CustomRayCastCar : public NewtonCustomJoint  
{
	...
	struct Tire 
	{
		...
		dFloat m_fixedRollingFriction;	   // coefficient of friction for idle tires (usually a very small value, but could be large for tire damage)	 
	...
	CustomRayCastCar(int maxTireCount, const dMatrix& chassisMatrix, NewtonBody* carBody);
	...
	protected:
//	void SetTireBrake (int index);
	void SetTireTorque (int index, dFloat torque);
	void SetTireSteerAngle (int index, dFloat angle);


	virtual dFloat GetNormalizedTractionAndBrakingForce (dFloat slipRatio) const;
	virtual dFloat GetNormalizedLateralForce (dFloat sideSlipAngleIndDegres) const;
	...
	void ApplyTireFrictionModel(Tire& tire, const dMatrix& chassisMatrix,
								const dVector& tireAxelVeloc, const dVector& tireAxelPosit, 
	...
	static void IntegrateTires (const NewtonJoint* me, dFloat timestep, int threadIndex);
