	...
	:NewtonCustomJoint(6, child, parent)
{
	...
	SetAngularLimis(-30.0f * 3.1416f / 180.0f, 30.0f * 3.1416f / 180.0f);
...
void CustomSlidingContact::GetInfo (NewtonJointRecord* info) const
{
	...
	if (m_limitsAngularOn) {
		...
		info->m_minAngularDof[0] = (m_minAngularDist - angle) * 180.0f / 3.1416f ;
		info->m_maxAngularDof[0] = (m_maxAngularDist - angle) * 180.0f / 3.1416f ;
	} else {
		info->m_minAngularDof[0] = -FLT_MAX ;
		info->m_maxAngularDof[0] =  FLT_MAX;
	}
