...
void CRocketLauncher::OnSpawn(sym::CVector3 Position, sym::CQuaternion Rotation)
{
	CInventoryItemBase::SetName("RocketLauncher");
...
void CRocketLauncher::OnRender()
{
	//Dot.Render();
}

void CRocketLauncher::OnDestroy()
{
}

bool CRocketLauncher::Fire(int FireMode)
{
	if(Owner && FireMode == FIRE_PRIMARY)
	{
		//Spawn projectile
		sym::CVector3 Start = Owner->GetPosition() + (Owner->GetForward() * ((Owner->GetLocalAABB().Size.y / 2.0f) + 50.0f));
		CProjectileBase *Projectile = Game->GetCurrentLevel()->Spawn<CProjectileBase>(Start, Owner->GetRotation());
