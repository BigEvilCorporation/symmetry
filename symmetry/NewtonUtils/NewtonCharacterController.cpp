#include "Newton/customJoints/CustomJointLibraryStdAfx.h"
#include "NewtonCharacterController.h"
...
//	SetMaxSlope (25.0f * 3.1416f / 180.0f);
	SetMaxSlope (45.0f * 3.1416f / 180.0f);
	...
	for (int i = 0; i < SENSOR_SHAPE_SEGMENTS; i ++) {
		dFloat x;
		dFloat z;
		x = radius * dCos (2.0f * 3.1416 * dFloat(i) / dFloat(SENSOR_SHAPE_SEGMENTS));
		z = radius * dSin (2.0f * 3.1416 * dFloat(i) / dFloat(SENSOR_SHAPE_SEGMENTS));
...
const NewtonCollision* NewtonPlayerController::GetVerticalSensorShape () const
{
	return m_verticalSensorShape;
}

const NewtonCollision* NewtonPlayerController::GetHorizontalSensorShape () const
{
	return m_horizontalSensorShape;
}

const NewtonCollision* NewtonPlayerController::GetDynamicsSensorShape () const
{
	return m_dynamicsCollisionShape;
}

void NewtonPlayerController::SetMaxSlope (dFloat maxSlopeAngleIndRadian)
{
	m_maxSlope = dCos (maxSlopeAngleIndRadian);
}

dFloat NewtonPlayerController::GetMaxSlope () const
{
	return dAcos (m_maxSlope);
}

void NewtonPlayerController::GetInfo (NewtonJointRecord* info) const
{
}

void NewtonPlayerController::SetVelocity (dFloat forwardSpeed, dFloat sideSpeed, dFloat heading)
{
	m_heading = heading;
...
unsigned NewtonPlayerController::ConvexStaticCastPrefilter(const NewtonBody* body, const NewtonCollision* collision, void* userData)
{
/*
...
unsigned NewtonPlayerController::ConvexDynamicCastPrefilter(const NewtonBody* body, const NewtonCollision* collision, void* userData)
{
	// for now just collide with static bodies
	dFloat mass; 
	dFloat Ixx; 
...
unsigned NewtonPlayerController::ConvexAllBodyCastPrefilter(const NewtonBody* body, const NewtonCollision* collision, void* userData)
{
	FilterData* filterData;
...
dFloat NewtonPlayerController::FindFloorCallback(const NewtonBody* body, const dFloat* hitNormal, int collisionID, void* userData, dFloat intersetParam)
{
	dFloat param;
...
void NewtonPlayerController::SubmitConstrainst (dFloat timestep, int threadIndex)
{
	//_ASSERTE (0);
